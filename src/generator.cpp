...
// Способ обработки окна сроков d_j.
// article нужен для воспроизводимости сравнения со статьёй, uniform -- для более чистой модели.
enum class due_mode {
	article,
	uniform,
};
...
instance generate_potts_instance_impl(int n, int p_min, int p_max, double due_range, double tardiness_factor, std::uint64_t seed, due_mode mode) {
	...
	long long total_processing_P = 0;
	for (int i = 0; i < n; ++i) {
		const int p = p_dist(rng);
		total_processing_P += p;
	...
	// Окно сроков строится от p(N) = sum_j p_j:
	// [p(N) * (1 - TF - RDD/2), p(N) * (1 - TF + RDD/2)].
	const double lower_factor = 1.0 - tardiness_factor - due_range * 0.5;
	const double upper_factor = 1.0 - tardiness_factor + due_range * 0.5;

	long long due_low = static_cast<long long>(static_cast<double>(total_processing_P) * lower_factor);
	long long due_high = static_cast<long long>(static_cast<double>(total_processing_P) * upper_factor);
	if (due_low > due_high) {
		std::swap(due_low, due_high);
	}

	const long long due_min = 0;
	const long long due_max = static_cast<long long>(std::numeric_limits<due_date_t>::max());

	if (mode == due_mode::uniform) {
		// Uniform-clamped:  сначала ограничиваем окно сроков допустимым диапазоном,
		// затем выбираем d_j из уже ограниченного окна.
		due_low = std::clamp(due_low, due_min, due_max);
		due_high = std::clamp(due_high, due_min, due_max);
		if (due_low > due_high) {
			std::swap(due_low, due_high);
		}
	...
	// Article-compatible: сначала выбираем значение из исходного окна,
	// затем обрезаем d_j до допустимого диапазона.
	std::uniform_int_distribution<long long> d_dist(due_low, due_high);
	for (job& j : inst.jobs) {
...
bool validate_potts_generation_config(const potts_generation_config& cfg, std::string* error) {
	...
	// Проверяем верхнюю границу p(N) до генерации, чтобы p_j и d_j затем безопасно
	// приводились к компактным типам processing_time_t и due_date_t.
