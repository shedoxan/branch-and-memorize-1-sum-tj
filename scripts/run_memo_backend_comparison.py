#!/usr/bin/env python3
"""Compare custom memo with std::unordered_map memo for adaptive_v3."""
...
DEFAULT_BACKENDS = "custom,std_unordered"
...
def write_summary(path: Path, rows: list[dict[str, str]]) -> None:
    backends = sorted({row.get("memo_backend", "") for row in rows})
    with path.open("w", encoding="utf-8") as f:
        f.write("# Memo Backend Comparison Summary\n\n")
        f.write(f"- rows: {len(rows)}\n")
        f.write(f"- objective_conflicts: {objective_conflicts(rows)}\n\n")
        f.write("| Backend | Rows | Status counts | Median ms | P90 ms | Median nodes | Median exact hit rate | Median memory bytes |\n")
        f.write("|---|---:|---|---:|---:|---:|---:|---:|\n")
        for backend in backends:
            rr = [row for row in rows if row.get("memo_backend") == backend]
            solved = [row for row in rr if row.get("status") == "SOLVED"]
            hit_rates = []
            for row in solved:
                queries = as_float(row.get("memo_exact_queries", ""))
                hits = as_float(row.get("memo_exact_hits", ""))
                hit_rates.append(hits / queries if queries > 0 else 0.0)
            f.write(
                f"| {backend} | {len(rr)} | {status_counts(rr)} | "
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Memo backend comparison runner.")
    ...
    parser.add_argument("--out-dir", default=str(root / "results" / "memo_backend_comparison"))
        ...
        for backend in backends:
            for r, t in pairs:
                cmd = [
                    ...
                    "--series", "hard",
                    "--model", "adaptive_v3",
                    "--hard-pairs", f"{fmt_float(r)}:{fmt_float(t)}",
                    "--n-values", str(n),
