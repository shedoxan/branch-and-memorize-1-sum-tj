    ...
    for key, rows in grouped.items():
        ...
        for row in candidates:
            if parse_float(row, "time_ms", math.inf) <= best_time * 1.000001:
                stats[row_model(row)].seed_wins += 1
...
def decide_elimination(n: int, stats: dict[str, ModelStageStats]) -> list[str]:
    ...
    best_penalized = min((s.median_penalized_time_ms for s in stats.values()), default=math.inf)
    ...
    for model, item in stats.items():
        ...
        elif threshold is not None and item.seed_wins == 0 and item.median_penalized_time_ms > threshold * best_penalized:
            item.decision = "drop"
            item.reason = f"median penalized time > {threshold:.1f}x best and zero wins"
    ...
    if not kept and stats:
        ...
        if not rescue_candidates:
            return []
        best_model = min(rescue_candidates, key=lambda s: (s.median_penalized_time_ms, -s.solved)).model
...
def write_stage_summary(path: Path, n: int, timeout_sec: int, stats: dict[str, ModelStageStats], conflicts: list[str], survivors: list[str]) -> None:
    rows = sorted(stats.values(), key=lambda s: (s.decision != "keep", s.median_penalized_time_ms, -s.seed_wins, s.model))
    with path.open("w", encoding="utf-8", newline="\n") as f:
        ...
        if conflicts:
            ...
            f.write("\n")
        f.write("## Ranking\n\n")
        f.write("| Model | Decision | Reason | Rows | Solved | Mismatch | OOT | OOM | ERR | Wins | Median ms | P90 ms | Penalized median ms | Median nodes | Median memory bytes |\n")
        f.write("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
        for s in rows:
            f.write(
                f"| {s.model} | {s.decision} | {s.reason} | {s.rows} | {s.solved} | "
                f"{s.objective_mismatches} | {s.oot} | {s.oom} | {s.errors} | {s.seed_wins} | "
                f"{s.median_time_ms:.3f} | {s.p90_time_ms:.3f} | {s.median_penalized_time_ms:.3f} | "
                f"{s.median_nodes:.0f} | {s.median_memory_bytes:.0f} |\n"
            )


def write_final_report(path: Path, completed_n: list[int], survivors: list[str], eliminated: dict[str, str]) -> None:
    with path.open("w", encoding="utf-8", newline="\n") as f:
        f.write("# Progressive Model Race Final Report\n\n")
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(description="Progressive model race runner for solver_bench.")
    ...
    parser.add_argument("--out-dir", default=str(root / "results" / "progressive_model_race"))
    ...
    for n in n_values:
        ...
        stats, conflicts = analyze_stage(n, active_models, all_rows, r_values, t_values, seeds, timeout_sec)
        if conflicts:
            write_stage_summary(out_dir / f"stage_n{n}_summary.md", n, timeout_sec, stats, conflicts, active_models)
        ...
        write_stage_summary(out_dir / f"stage_n{n}_summary.md", n, timeout_sec, stats, conflicts, survivors)
        completed_n.append(n)
        active_models = survivors

    write_final_report(out_dir / "final_report.md", completed_n, active_models, eliminated)
